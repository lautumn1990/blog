...
```sh
# 生成CA根证书
openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout /tmp/my_ca.key -out /tmp/my_ca.crt -subj "/CN=myca/O=myca"
```
...
echo subjectAltName = DNS:${MY_HOST} >> /tmp/extfile.cnf
echo extendedKeyUsage = serverAuth >> /tmp/extfile.cnf

openssl genrsa -out /tmp/${MY_SECRET}.pem 4096
openssl req -subj "/CN=${MY_HOST}" -sha256 -new -key /tmp/${MY_SECRET}.pem -out /tmp/${MY_SECRET}.csr
openssl x509 -req -days 365 -sha256 -in /tmp/${MY_SECRET}.csr -CA /tmp/my_ca.crt -CAkey /tmp/my_ca.key -CAcreateserial -out /tmp/${MY_SECRET}-cert.pem -extfile /tmp/extfile.cnf

cat /tmp/${MY_SECRET}-cert.pem /tmp/my_ca.crt > /tmp/${MY_SECRET}-chain.pem
# 创建secret
kubectl create secret tls ${MY_SECRET} -n ${MY_NAMESPACE} --key /tmp/${MY_SECRET}.pem --cert /tmp/${MY_SECRET}-chain.pem

# 检验证书
openssl x509 -noout -text -in /tmp/${MY_SECRET}-chain.pem
```
